"""
Plugin : Revue des Droits Opérateurs – BASTION eIDAS (DSO-LOG-19)
Croise l'export CSV de la console BASTION eIDAS avec BaseLIR.
"""
...
# profile_name (lowercase) → critères attendus dans BaseLIR
CONFORMITY_RULES = {
...
def _is_service_account(username: str, utilisateur: str) -> bool:
    ...
    if "." in utilisateur or "_" in utilisateur:
        return True
    words = set(utilisateur.lower().split())
...
def _parse_nom_prenom(utilisateur: str) -> tuple[str, str]:
    """Dernier mot = NOM (nom de famille), le reste = PRENOM."""
...
def _fetch_lir(lir_url: str, lir_key: str) -> dict:
    """Retourne {norm(nom_prenom): [{role, domaine, direction, service}, ...]}"""
...
async def execute(form, config: dict, lir_url: str, lir_key: str, control_date: str) -> dict:
    ...
    if not bastion_file or not getattr(bastion_file, "filename", None):
        raise ValueError("Le fichier export BASTION eIDAS est requis.")
    ...
    for row in rows:
        username    = _get_col(row, "Username", "Compte_utilisateur")
        utilisateur = _get_col(row, "Full name", "utilisateur", "Full_name")
        profile     = _get_col(row, "Profile", "profile_name")
    ...
    return {
        ...
        "resume": {
            "BASTION eIDAS": {"total": total, "conformes": conformes, "ecarts": n_ecarts},
...
def compute_taux(result: dict) -> float:
    b      = result.get("resume", {}).get("BASTION eIDAS", {})
    total  = b.get("total", 0)
...
def build_commentaire(result: dict) -> str:
    ...
    if nb == 0:
        return (f"Contrôle BASTION eIDAS du {date_s} : {total} compte(s) nominatif(s) "
                f"contrôlé(s), tous conformes.")
    return (f"Contrôle BASTION eIDAS du {date_s} : {nb} écart(s) sur {total} "
